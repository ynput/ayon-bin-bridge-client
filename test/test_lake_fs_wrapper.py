...
from ayon_automator.AyonCiCd import helpers
...
ctl = wrapper.LakeCtl(
    base_uri_oberwrite="https://lake.ayon.cloud",
...
def test_list_repo_objects():
    ...
    if not test_object_on_the_repo in repo_list:
        helpers.FAIL(
            f"did not list file {test_object_on_the_repo} <- this is a test object uploaded to the lake fs test repo"
        )
    print()
...
def test_clone_local():
    ...
    path = ctl.clone_local(
        progress_obj=None,
        repo_branch_uri=lake_fs_prj,
        dist_path=dist_path,
        print_stdout=True,
    )
...
def test_clone_elemente():
    ...
    element = ctl.clone_element(
        progress_obj=None, lake_fs_object_uir=lake_fs_object, dist_path=dist_path
    )
